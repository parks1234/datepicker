package com.hbbcw.zlapp.custom.datepicker.views;
...
import com.hbbcw.zlapp.custom.datepicker.calendars.DPCManager;
import com.hbbcw.zlapp.custom.datepicker.cons.DPMode;
import com.hbbcw.zlapp.custom.datepicker.decors.DPDecor;
import com.hbbcw.zlapp.custom.datepicker.entities.DPInfo;
import com.hbbcw.zlapp.custom.datepicker.themes.DPTManager;
import com.hbbcw.zlapp.custom.datepicker.utils.DateUtil;
...
public class MonthView extends View {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mSlideMode == SlideMode.HOR) {
                    ...
                    if (isshowHoliday && lastPointX < event.getX()) {
                     
                        if (centerYear == DateUtil.getYear()){

                            if (centerMonth - 1 < DateUtil.getMonth())
                                mSlideMode = SlideMode.NOUP;
                        } else   if (centerYear - 1 <DateUtil.getYear()){
                            mSlideMode = SlideMode.NOUP;
                        }else
                    ...
                    if (isshowHoliday && lastPointY < event.getY()) {
                       
                         if (centerYear-1 == DateUtil.getYear()){
                            if (centerMonth - 1 < DateUtil.getMonth())
                                mSlideMode = SlideMode.NOUP;
                        }else if (centerYear - 1 <DateUtil.getYear())
                           {
            ...
            case MotionEvent.ACTION_UP:
                ...
                } else if (mSlideMode == SlideMode.NOUP) {
    ...
    void setisshowHoliday(boolean isshowHoliday) {
        ...
        for (int i = 0; i < tmp.length; i++) {
            for (int j = 0; j < tmp[i].length; j++) {
                ...
                if (region.contains(x, y)) {
                    ...
                    if (isshowHoliday) {
                        if (centerYear <= DateUtil.getYear())
                            if (centerMonth <= DateUtil.getMonth())
                                if (Integer.parseInt(mCManager.obtainDPInfo(centerYear, centerMonth)[i][j].strG) < DateUtil.getDAy()){
                                    return;
                                }
                    ...
                    } else if (mDPMode == DPMode.TWOSELECT) {
                        if (regions.contains(region)) {
                            regions.remove(region);
                        } else {
                            regions.add(region);
